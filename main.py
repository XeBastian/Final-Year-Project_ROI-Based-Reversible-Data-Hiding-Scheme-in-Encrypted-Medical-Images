...
def randomise_sides():
    ...
    keep_looping = True
    side_data = []
    while keep_looping:  # randomise 3 sides
    ...
    for i1 in n:
        ...
        if i1 == 0:
            side_data.append([0, 0, mode])
        elif i1 == 1:
            side_data.append([0, 1, mode])
        elif i1 == 2:
            side_data.append([1, 0, mode])
        else:
            side_data.append([1, 1, mode])
    return side_data
...
def get_side2(curr_row, pixel, channel):
    global img, rows, columns
    t_row = rows - curr_row - 1
    if side2Mode == 0:
        t_col = columns - curr_row - 2 - pixel
    else:
        t_col = curr_row + 1 + pixel
    return img[t_row][t_col][channel] % 2
        ...
        for i1 in range(4):
            for j1 in range(4):
                ...
                if sides[j1] == 0:
                    ...
                    retrievedTextRef += str(get_side2(currRow, temp1 // 3, temp1 % 3))
                else:
...
for j in range(int(len(encryptionIp) / 32)):
    ...
    rounds: list[Union[list[list[int]], list[str]]] = [apply_round_key(keys[0], plain)]
...
# ROI BASED RDH SCHEME FOR ENCYPTED MEDI IMAGES
# TODO: Add Image /Region Detection only